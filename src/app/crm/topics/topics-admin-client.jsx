...
export default function TopicsAdminClient() {
  ...
  async function remove(i) {
    ...
    setBusy(`del-${i}`);
    try {
      const res = await fetch(`/api/admin/topics/${i}`, { method: "DELETE" });
      const data = await res.json();
            ...
            <table className="table table-zebra w-full">
              ...
              <tbody>
                {loading ? Array.from({ length: 6 }).map((_,i)=>(
                  ...
                  <tr key={`${i}-${t.slice(0,20)}`}>
                    <td>{i + 1}</td>
                    <td className="max-w-[48rem] truncate" title={t}>{t}</td>
                    <td className="text-right">
                      <div className="join">
                        <button className="btn btn-sm join-item" onClick={() => move(i, "up")} disabled={i===0 || busy.startsWith("move-")}>
                          ↑
                        </button>
                        <button className="btn btn-sm join-item" onClick={() => move(i, "down")} disabled={i===topics.length-1 || busy.startsWith("move-")}>
                          ↓
                        </button>
