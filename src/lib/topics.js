...
async function readFromGitHub(filePath) {
  const url = `https://api.github.com/repos/${GH.owner}/${GH.repo}/contents/${encodeURIComponent(
    filePath
...
async function writeToGitHub(filePath, content, message, sha) {
  ...
  if (!res.ok) {
    throw new Error(
      `GitHub write failed ${res.status}: ${await res.text()}`
    );
  }
...
export async function getNextTopic() {
  if (USE_GITHUB) {
    ...
    await writeToGitHub(
      ...
      updated,
      next
        ? `chore(topics): consume "${next}"`
        : "chore(topics): consume(empty)",
      gh.sha || undefined
    );
    return next || null;
  }
